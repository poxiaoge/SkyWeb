...
class Comment(db.Model):
    ...
    def on_changed_body(target, value, oldvalue, initiator):
        allowed_tags = ['a', 'b', 'abbr', 'acronym', 'code', 'em', 'i', 'strong']
    ...
    def from_json(json_comment):
        ...
        if body is None or body=="":
            raise ValidationError('Comment body must be valid!')
...
class Post(db.Model):
    ...
    def from_json(json_post):
        ...
        if body is None or body == "" or title is None or title == "":
            raise ValidationError('Post must have a valid title and body!')
...
class User(UserMixin, db.Model):
    ...
    img_url_postfix = db.Column(db.String(64),default="default.jpg")
    ...
    @staticmethod
    def get_img_url_prefix():
        return url_for('main.index')+'static/thumbnail/'
    ...
    def model_init():
        for user in User.query.all():
            ...
            if not user.is_following(user):
                user.follow(user)
                flag=True
            if not user.img_url_postfix:
                user.img_url_postfix="default.jpg"
                flag=True
            if flag:
                db.session.add(user)
        db.session.commit()
    ...
    def generate_confirm_token(self, expires_in=3600):
        s = Serializer(current_app.config.get('SECRET_KEY'), expires_in)
        return s.dumps({'confirm': self.id})

    def generate_change_email_token(self, dump_dict, expires_in=3600):
        s = Serializer(current_app.config.get('SECRET_KEY'), expires_in)
        return s.dumps(dump_dict)
    ...
    def is_administer(self):
        return self.have_permission(Permission.ADMINISTER)
    ...
    def insert_roles():
        roles = {
            ...
            'moderator': (Permission.FOLLOW |
                          ...
                          Permission.MANAGE_COMMENT, False),
